...
Param_mediation_projection_survival <- R6Class(
  ...
  public = list(
    initialize = function(observed_likelihood, intervention_list_treatment, intervention_list_control, static_likelihood = NULL, n_resampling = NULL) {
      ...
      private$.outcome_node <- last(temp_node_names)
      ...
      obs_data <- tmle_task$data %>%
        # as.data.frame %>%
        dplyr::select(-c(id, t)) %>% dplyr::select(!starts_with("delta"))
      ...
      if (!is.null(n_resampling)) {  # use expanded Monte Carlo samples to train the HAL projection
        ...
        temp_task <- tmle3_Task_drop_censored$new(temp_input, tmle_task$npsem, id = "id", time = "t")
    ...
    estimates = function(tmle_task = NULL, fold_number = "full", est_n_samples = 5000, final = F, seed = NULL) {
      ...
      if(final) {
        ...
        all_possible_RZLY_1 <- expand_values_size(variables = obs_variable_names,
        ...
        all_possible_RZLY_0 <- all_possible_RZLY_1
      ...
      #   observed_task <- tmle_task
      #   # cf_likelihood_treatment <- CF_Likelihood$new(self$observed_likelihood, self$intervention_list_treatment)
      #   # cf_likelihood_control <- CF_Likelihood$new(self$observed_likelihood, self$intervention_list_control)
      #
      ...
      #   # mc_task_trt <- tmle3_Task$new(mc_data, observed_task$npsem[baseline_node], id = "id", t = "t", long_format  = observed_task$long_format)
      #   # mc_task_ctrl <- tmle3_Task$new(mc_data, observed_task$npsem[baseline_node], id = "id", t = "t", long_format  = observed_task$long_format)
      #   time_ordering <- setdiff(time_ordering, baseline_node)
      #   for(node in time_ordering) {
      #     if (node %in% nodes_A) {
      #       if (length(grep("^A_C_", node))>0) {
